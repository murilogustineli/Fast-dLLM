                    ...
                    while True:
                        ...
                        if use_block_cache:
                            ...
                            should_recompute = (
                                ...
                                or (reuse_counter % reuse_k == 0)   # every k-th step refresh
                ...
                if past_key_values is not None:
                    ...
                    # Case B: nested caches list (e.g., past_key_values.caches[i].k_cache / v_cache)
                        ...
                        for layer_cache in past_key_values.caches:
                            if hasattr(layer_cache, "k_cache") and hasattr(layer_cache, "v_cache"):
                    ...
                    if "block_past_key_values" in locals() and block_past_key_values is not None:
                        if hasattr(block_past_key_values, "key_cache") and hasattr(block_past_key_values, "value_cache"):
                            Lb = len(block_past_key_values.key_cache)
                            for layer_id in range(Lb):
                                block_past_key_values.key_cache[layer_id]   = block_past_key_values.key_cache[layer_id][keep]
                                block_past_key_values.value_cache[layer_id] = block_past_key_values.value_cache[layer_id][keep]
